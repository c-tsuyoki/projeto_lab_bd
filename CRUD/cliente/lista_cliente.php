...
<html lang="pt-br">
	...
	<body>
		...
		while($row = mysqli_fetch_assoc($resultado_usuarios)){
			...
			echo $row['nome'] . "</td><td>". $row['endereco'] . "</td><td>". $row['complemento']. "</td><td>". $row['bairro']."</td><td>" .$row['cidade'] . "</td><td>". $row['estado'] . "</td><td>". $row['cpf_cnpj']. "</td><td>" .$row['rg'] . "</td><td>". $row['email'] . "</td><td>". $row['telefone']."</td><td>" .$row['celular']. "</td><td>" .$row['cep']. "</td><td> <a href='tela_edit_cliente.php?id=". $row['id'] . "'>Editar</a>";
		...
		echo "<a href='lista_cliente.php?pagina=$quantidade_pg'>Última</a>";
		?>		
	</body>
</html>